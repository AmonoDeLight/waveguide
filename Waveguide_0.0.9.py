...
def plotting():
    ...
    #def TE10_H_XY_neg(x, y):
        #return sin(kappaX * x*2-hh-4.9) * exp(hh * tan(w * t) * y)*-1
    ...
    def TE10_E_YZ_neg(y, z):
       return abs((cos(w * t - hh * y/2-20))) *2  #abs((cos(w * t - hh * y/2-20))) *-2  #abs(pow(-1, tt) * (cos(w * t - hh * y/3-20)))*-2
           ...
           #pow(-1, tt) * (cos(w * t - hh * y))
            #(cos(kappaX*y)*cos(kappaX*y)*cos(kappaX*y)) * sin(w * t)
        #if sin(w*t)>0:
         #   return abs(cos(w*t+hh*y))*sin(w*t)
        #else:
         #   return -(abs(cos(w * t + hh * y)))*sin(w*t)

        #(w / (kappaX * cc))*sin(kappaX*y)* exp(hh * tan(w * t) * y)
        #cos( w * t - hh * y) * (w / (kappaX * cc)) #* sin(kappaX * y)  #0.1 - cos(w * t - hh * y + 0.2) * (w / (kappaX * cc))
    ...
    if f > f_kr:
        ...
        if combobox.get() == "TE":  # ïðîâåðÿåì òèï âîëí è äàëüøå ñ÷èòûâàåì ìîäó
            ...
            if n != 0 and m != 0:
                ...
                XZ.contour(x1, y1, TE10_E_YZ_pos(x1, y1), linspace(-1, 1, k), colors='b')
                XZ.contour(x1, y1, TE10_E_YZ_neg(x1, y1), linspace(-1, 1, k), colors='b')
                XZ.set_xlabel('z')
                XZ.set_ylabel('x')
            else:
...
scale = Scale(window, orient="horizontal", from_=0, to=50,  font=LARGE_FONT, bg="#1A1A1D", fg="#bbbbbf", command=onScale)
